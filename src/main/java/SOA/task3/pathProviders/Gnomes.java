...
public class Gnomes {
	...
	private void addHateoasLinks(Gnome gnome) {
	    ...
	    boolean selfLinkExists = gnome.getLinks().stream()
	        .anyMatch(link -> "self".equals(link.getRel()));

	    if (!selfLinkExists) {
	        Link selfLink = Link.fromUri(
	    ...
	    boolean creatorLinkExists = "creator".equals(gnome.getCreator_link() != null ? gnome.getCreator_link().getRel() : null);
	    ...
	    if (gnome.getOwnerId() != -1) {
	        boolean ownerLinkExists = "owner".equals(gnome.getOwner_link() != null ? gnome.getOwner_link().getRel() : null);
